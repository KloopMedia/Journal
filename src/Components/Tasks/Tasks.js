...
const Tasks = (props) => {
	...
	useEffect(() => {
		if (currentUser) {
			...
			firebase.firestore().collection("tasks").where("quick", "==", true).get()
				.then((querySnapshot) => {
					querySnapshot.forEach((doc) => {
						if (!doc.data().is_complete) {
							console.log(doc.id, " => ", doc.data());
							quick.push({id: doc.id, ...doc.data()})
						}
					});
				})
				.then(() => {
	...
	return (
		<Grid container justify="center" alignItems="center" direction="column">
			...
			<div className={classes.root}>
				<Paper position="static" color="default">
					<Tabs value={value} onChange={handleChange} variant="fullWidth" centered
						...
						<Tab label="Быстрые задания" {...a11yProps(0)}/>
						<Tab label="Выполненные" {...a11yProps(1)}/>
						<Tab label="Невыполненные" {...a11yProps(2)}/>
					</Tabs>
				</Paper>
			</div>
			...
			<TabPanel value={value} index={0}>
				{quickTasks && quickTasks.map((task, i) => (
					<Grid key={'quick_task_' + i} style={{padding: 10}}>
						<QuickTask case_id={task.case_id} case_type={task.title}
								   task_type={task.case_stage_id}/>
					</Grid>
				))}
			</TabPanel>
		</Grid>
