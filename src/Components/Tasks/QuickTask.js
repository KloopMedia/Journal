...
const QuickTask = (props) => {
	...
	const [lockButton, setLock] = useState(true)
	...
	const {task_type, case_type, case_id} = props;

	const {currentUser} = useContext(AuthContext);
	const id = props.case_id;
	...
	useEffect(() => {
		const getQuestions = async (taskID) => {
			...
			firebase.firestore().collection("tasks").doc(taskID).collection("questions").get()
				.then((querySnapshot) => {
		...
		const getForms = async () => {
			...
			let sf = sq.map((el, i) => {
				...
				return <Form key={el.questionId + '_' + i} question={el.data} index={i} response={response}
							 returnAnswer={returnAnswer} locked={true} askFeedback={true}
							 saveQuestionFeedback={saveQuestionFeedback} id={el.questionId}
							 prevTaskId={sameCaseTasks[0].id}/>
			})

			sf.push(<div key={"div_divider_stripped"} style={{
				height: 25,
				margin: '20px 0',
				background: 'repeating-linear-gradient( 45deg, white, white 10px, grey 10px, grey 25px)'
			}}><br/></div>)
	...
	useEffect(() => {
		let task;
		firebase.firestore().collection("schema").doc("structure").collection("cases").doc(case_type).collection("stages").get()
			.then((querySnapshot) => {
				querySnapshot.forEach((doc) => {

					task = {id: doc.id, ...doc.data(), disabled: false}
					// console.log({id: doc.id, ...doc.data(), disabled: false})
	...
	const sendRequest = (type, id, case_type, task_type, case_id) => {
		console.log({
			taskType: task_type,
			status: "pending",
		...
		/*firebase.firestore().collection("task_requests").doc(currentUser.uid).collection("requests")
			.add({
			taskType: task_type,
					status: "pending",
					user: currentUser.uid,
	...
	const releaseTask = () => {
		firebase.firestore().collection("tasks").doc(id).collection("user_editable").doc("user_editable").update({status: 'open'})
			.then(() => firebase.firestore().collection("tasks").doc(id).collection("user_editable").doc("user_editable").update({
				status: 'released',
				release_status: feedbackValue.reason,
				release_description: feedbackValue.text
			})
				.then(() => {
					setDialog(false)
					setLock(true)
	...
	return (
		currentUser ?
			<Grid style={{padding: 30}}>
				...
				{dialogType === 'send' &&
				<Dialog state={dialogState} handleClose={handleDialogClose} title={"Отправить задание?"}
						content={"Вы собираетесь отправить задание. Это значит, что вы больше не сможете изменять ответы."}
				...
				<Grid container style={{padding: 20}} justify="center">
					{(isAssigned) ? !lockButton &&
						<div>
							<Button variant="outlined" disabled={lockButton}
									style={{borderWidth: 2, borderColor: "#003366", color: '#003366', margin: 5}}
									onClick={() => saveToFirebase(false)}>Сохранить</Button>
							<Button variant="outlined" disabled={lockButton}
									style={{borderWidth: 2, borderColor: "red", color: 'red', margin: 5}}
						...
						<div>
							<Button variant="contained" /*disabled={disabled}*/ color="primary"
									size="small"
									onClick={() => sendRequest(id, task_type, case_type, task.type, case_id)}>
								{"Получить"}
							</Button>
						</div>
							: (dialogState % dialogType == "send") ? <Typography color="error">
							Сдано
						</Typography>:false
